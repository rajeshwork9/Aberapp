...
type PickedImage = {
  ...
  height?: number;
  fileSize?: number;
  base64?: string; // from image-picker
};
...
type FormValues = {
  ...
  companyLicenseNo: string;       // UI label: "Enter Company Licence Number *"
  companyLicenseNameAr: string;   // UI label: "Company Licence Name Arabic *"
  noOfTrucks: string;
  email: string;
  mobile: string;
...
const validationSchema = Yup.object().shape({
  ...
  companyLicenseNameAr: Yup.string().trim().required('Company License Name Arabic is required'),
  companyLicenseNo: Yup.string().trim().required('Company License Number is required'),
...
const AddVehicle: React.FC = () => {
  ...
  const handleImageAsset = (asset: Asset | undefined): PickedImage | null => {
    ...
    // Note: when using includeBase64: true, "base64" is present for immediate use
  };

  const chooseCamera = async (): Promise<PickedResult | null> => {
    const res = await launchCamera({
      mediaType: 'photo',
      includeBase64: true, // get base64 for images
      saveToPhotos: false,
      quality: 0.9,
    });
  ...
  const openSheetForField = (
    ...
    if (Platform.OS === 'ios') {
      if (pdfOnly) {
        const options = ['Documents (PDF only)', 'Cancel'];
        ActionSheetIOS.showActionSheetWithOptions(
          { options, cancelButtonIndex: 1 },
          async (idx) => {
            if (idx === 0) await handlePick('documents');
          }
        );
      } else {
  ...
  const stripFilePrefix = (uri: string) =>
    uri.startsWith('file://') ? uri.replace('file://', '') : uri;

  const fileToBase64 = async (uri: string) => {
    // Best effort: RNFS can usually read both file:// and content:// on Android.
    // If you find a content:// that fails, use a doc picker option that returns a fileCopyUri,
    // or copy to cache then read.
    try {
      return await RNFS.readFile(stripFilePrefix(uri), 'base64');
    } catch (e) {
      // Last resort, try reading the raw uri (helps some content:// cases)
      return await RNFS.readFile(uri, 'base64');
    }
  };

  const toApiFile = async (picked: PickedResult | null) => {
    if (!picked) return null;
    const fileName =
      picked.name || picked.uri.split('/').pop() || 'file';
    if (picked.kind === 'image' && (picked as PickedImage).base64) {
      return { fileName, base64Data: (picked as PickedImage).base64! };
    }
    const base64Data = await fileToBase64(picked.uri);
    return { fileName, base64Data };
  };

  const submitCase = async (values: FormValues, _helpers: FormikHelpers<FormValues>) => {
  ...
  const submitToApi = async () => {
    ...
    try {
      ...
      const companyDoc = await toApiFile(lastValues.companydocument);
      const companyLicenceDoc = await toApiFile(lastValues.companylicensedocument);
      const truckLicenceDoc = await toApiFile(lastValues.trucklicensedocument);

      const payload = {
        companyLicenseNameEn: lastValues.companyLicenseNameEn,
        companyLicenseNameAr: lastValues.companyLicenseNameAr, // mapped from Arabic name field
        companyLicenseNo: lastValues.companyLicenseNo,         // mapped from "Licence Number" field
        companyDetails: lastValues.companyDetails,
        noOfTrucks: Number(lastValues.noOfTrucks),
        email: lastValues.email,
        mobile: lastValues.mobile,
        sourceID: 3, // static as per your example
        companyDoc: companyDoc!,
        companyLicenceDoc: companyLicenceDoc!,
        truckLicenceDoc: truckLicenceDoc!,
      };
            ...
            <ScrollView>
              <View style={styles.container}>
                <View style={styles.innerContainerPad}>
                  ...
                  <View style={styles.step1}>
                    ...
                    <View style={styles.formGroup}>
                      <TextInput
                        ...
                        placeholder="Company Licence Name (English) *"
                      ...
                      />
                      {touched.companyLicenseNameEn &&
                        errors.companyLicenseNameEn && (
                    ...
                    <View style={styles.formGroup}>
                      <TextInput
                        ...
                        textColor="#fff"
                        theme={{ colors: { primary: '#FF5400' } }}
                        value={values.companyLicenseNameAr}
                        onChangeText={handleChange('companyLicenseNameAr')}
                        onBlur={handleBlur('companyLicenseNameAr')}
                      />
                      {touched.companyLicenseNameAr &&
                        errors.companyLicenseNameAr && (
                          <Text style={styles.errorText}>
                            {errors.companyLicenseNameAr}
                          </Text>
                        )}
                  ...
                  <View style={styles.step2}>
                    ...
                    <Card style={styles.cardItemMain}>
                      ...
                      <View style={styles.bulletText}>
                        <Text style={styles.bulletT}>{'\u2022'}</Text>
                        <Text style={styles.textB}>
                          Please Refer The Sample Documents Before Attaching The
                          Documents. Invalid Documents Will Be Rejected After The
                      ...
                      <View style={styles.bulletText}>
                        <Text style={styles.bulletT}>{'\u2022'}</Text>
                        <Text style={styles.textB}>
                          For More Than One Truck, Please Upload All Truck
                          License Document In A Single PDF File As Shown In The
                          Sample Below. Missing A Document Will Result To
                          Rejection Of The Registration After Payment.
                        </Text>
                      </View>
                    </Card>

                    {/* Company Licence Document */}
                    <Text style={styles.labelFile}>Company License Document *</Text>
                    <Card style={styles.chooseFileCard}>
                      <Button
                        mode="elevated"
                        textColor="#000"
                        style={styles.chooseFileBt}
                        disabled={!!values.companylicensedocument} // single file only
                        onPress={() =>
                          openSheetForField('companylicensedocument', setFieldValue)
                        }
                      >
                        {values.companylicensedocument ? 'File Selected' : 'Choose File'}
                      </Button>
                      ...
                      <View style={styles.fileRow}>
                        <Text style={styles.fileName}>
                          {values.companylicensedocument?.name ?? 'No file selected'}
                        </Text>
                        {values.companylicensedocument && (
                          <Pressable
                            onPress={() => setFieldValue('companylicensedocument', null)}
                          >
                    ...
                    <Card style={styles.chooseFileCard}>
                      <Button
                        ...
                        textColor="#000"
                        style={styles.chooseFileBt}
                        disabled={!!values.companydocument}
                        onPress={() =>
                          openSheetForField('companydocument', setFieldValue)
                        }
                      >
                        {values.companydocument ? 'File Selected' : 'Choose File'}
                      </Button>
                      ...
                      <View style={styles.fileRow}>
                        <Text style={styles.fileName}>
                          {values.companydocument?.name ?? 'No file selected'}
                        </Text>
                        {values.companydocument && (
                          <Pressable
                            onPress={() => setFieldValue('companydocument', null)}
                          >
                            <Text style={styles.deleteLink}>Delete</Text>
                          </Pressable>
                        )}
                    ...
                    {/* Truck Licence Documents (PDF Only) */}
                    <Text style={styles.labelFile}>
                      Truck Licence Documents (PDF Only) *
                    </Text>
                    <Card style={styles.chooseFileCard}>
                      <Button
                      ...
                      <View style={styles.fileRow}>
                        <Text style={styles.fileName}>
                          {values.trucklicensedocument?.name ?? 'No file selected'}
                        </Text>
                        {values.trucklicensedocument && (
                          <Pressable
                            onPress={() => setFieldValue('trucklicensedocument', null)}
                          >
                            <Text style={styles.deleteLink}>Delete</Text>
                          </Pressable>
                        )}
                    ...
                    <View style={styles.buttonRow}>
                      <Button
                        mode="outlined"
                        textColor="#FF5A00"
                        style={styles.clearBt}
                        onPress={() => handleSubmit()} // triggers validation + summary
                      >
                        Save
                      </Button>
                      <Button
                  ...
                  {/* ========== STEP 3: SUMMARY (appears after Save) ========== */}
                  {confirmSubmit && (
                    <View style={styles.step3}>
                      <Text style={styles.sectionTitle}>3. Summary</Text>
                      <Card style={styles.cardItemMain}>
                        <SummaryRow label="Company Name (EN)" value={values.companyLicenseNameEn} />
                        <SummaryRow label="Company Name (AR)" value={values.companyLicenseNameAr} />
                        <SummaryRow label="Licence Number" value={values.companyLicenseNo} />
                        <SummaryRow label="Truck Count" value={values.noOfTrucks} />
                        <SummaryRow label="Email" value={values.email} />
                        <SummaryRow label="Mobile" value={values.mobile} />
                        <SummaryRow label="Company Details" value={values.companyDetails} />
                        <SummaryRow
                          label="Company Licence Document"
                          value={values.companylicensedocument?.name ?? '—'}
                        />
                        <SummaryRow
                          label="Company Document"
                          value={values.companydocument?.name ?? '—'}
                        />
                        <SummaryRow
                          label="Truck Licence Document (PDF)"
                          value={values.trucklicensedocument?.name ?? '—'}
                        />
                      </Card>
                    </View>
                  )}
                ...
                {Platform.OS === 'android' && androidSheetVisible && (
                  <RNModal visible transparent animationType="fade">
                    <Pressable
                      ...
                      onPress={() => setAndroidSheetVisible(null)}
                    />
                    <View style={styles.sheet}>
                      <Text style={styles.sheetTitle}>Select Source</Text>

                      {androidSheetVisible.field !== 'trucklicensedocument' && (
                        <>
                          <Pressable
                            style={styles.item}
                            onPress={async () => {
                              ...
                              if (picked)
                                setFieldValue(androidSheetVisible.field, picked);
                    ...
                    <Portal>
                      ...
                      >
                        <View style={styles.confirmAlert}>
                          ...
                          >
                            The data and attachments will be saved. You won't be
                            able to update after saving.
                          </Text>
                          <View style={styles.buttonRow}>
                            <Button
                              mode="contained"
...
const styles = StyleSheet.create({
  ...
  deleteLink: { color: '#ff5555', textDecorationLine: 'underline', marginLeft: 8 },
});